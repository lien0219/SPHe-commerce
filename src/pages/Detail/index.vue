        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们的路由，从A路由跳转到B路由，这里加入购物车，进行路由跳转之前，发请求
...
export default {
  ...
  methods: {
    ...
    //加入购物车的回调函数
    async addShopcar() {
      //1：在点击购物车这个按钮的时候，做的第一件事情，将参数当服务器（发请求），通知服务器加入购物车的产品是谁
  //this.$store.dispatch('addOrderUpdateShopCart')，说白了，它是在调用vuex仓库中的addOrderUpdateShopCart，方法加上asyc，返回一定是Promise      
     //2:你需要知道这次请求成功还是失败，如果成功进行路由跳转，如果失败，需要给用户提示
      try {
        //成功
        await this.$store.dispatch("addOrderUpdateShopCart", {skuId: this.$route.params.skuid,skuNum: this.skuNum});
       //3：进行路由跳转
